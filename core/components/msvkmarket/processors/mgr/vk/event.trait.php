...
trait msVKMarketVKEventTrait
{
    ...
    public function addItem(array $item, $id_group, array $albums_id, $category_id)
    {
        ...
        //$this->modx->log(1, print_r($album_id, true));
        //$this->modx->log(1, print_r($category_id, true));
     ...
     * @return string
     */
    public function updItem(array $item, $id_group, $albums_id, $category_id)
    {

        $this->modx->log(1, 'update item');
    ...
    public function vkImport($item, $group, $action){
        ...
        if (empty($action)) {
            $this->modx->log(1, '[msVKMarket] Не переданно действие! ' . __METHOD__);
            return array(
                'success' => false
            );
        }
